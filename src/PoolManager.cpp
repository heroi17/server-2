...
MANAGER_INIT_ERROR PoolManager::INIT(uint32_t capacity, uint32_t bytesSizeOfType)
{
	if (capacity == 0 || bytesSizeOfType == 0) return MANAGER_INIT_ERROR::INVALID_INIT_DATA;
	std::lock_guard lg(allocMutex_);
	uint32_t BLOCK_SIZE_NEW = sizeof(LinkedListNode) + bytesSizeOfType;
...
bool PoolManager::freeObj(void* freeData)
{
	...
	uint32_t index = bytesBetween(toDeleteNode, this->dataPtr) / BLOCK_SIZE;
...
void* PoolManager::operator[](uint32_t index_)
{
	if (index_ >= capacity_) return nullptr;
	return (void*)(((LinkedListNode*)adress(this->dataPtr, index_, BLOCK_SIZE)) + 1);
}

bool PoolManager::getIndex(uint32_t& returnId, const void *const ptrObj_) const
{
	std::lock_guard lg(allocMutex_);
	...
	uint32_t index = bytesBetween(nodePtr, this->dataPtr) / BLOCK_SIZE;
	returnId = index;
	return true;
}

uint32_t PoolManager::getCapacity() const
{
...
void PoolManager::initLinkedList_()
{
	...
	std::uint32_t BLOCK_SIZE = sizeof(LLN) + objectSize_;
	// First elem init.
